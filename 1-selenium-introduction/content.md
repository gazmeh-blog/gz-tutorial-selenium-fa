## آشنایی با ابزار سلنیوم برای خودکارسازی تست‌های عملکردی
...
## مجموعه ابزار سلنیوم (Selenium Test Suite)
...
## سیاست منشأ یکسان (same-origin policy)
...
### مزایا: 
* **اجرای موازی آزمون‌های مختلف امکان‌پذیر است زیرا Selenium RC از Selenium Grid پشتیبانی می‌کند.**
...
شینیا کاساتانی(Shinya Kasatani) از ژاپن با انجام پروژه‌ای در مورد Selenium IDE در سال 2006 به ایجاد آن کمک کرد. سلنیوم IDE یک افزونه مرورگر است که از طریق قابلیت ضبط و اجرا جهت خودکارسازی مرورگرها استفاده می‌شود.
...
## انواع آزمون‌هایی که معمولاً با سلنیوم خودکارسازی می‌شوند عبارت‌اند از:
...
## مقایسه Selenium وQTP (که در حال حاضر UFT نامیده می‌شود)
QTP مخفف Quick Test Professional است که ابتدا توسط Mercury Interactive توسعه داده شد. بعد از مدتی توسط HP (Hewlett Packard) خریداری شد و آنها آن را UFT (Unified Functional Testing) نامیدند. UFT با HP QTP و HP Service Test در یک بسته نرم‌افزاری واحد ترکیب شد که ظاهراً تا سال 2016 در دسترس بود تا اینکه کل بخش به Microfocus فروخته شد.
...
### مزایای Selenium نسبت به QTP:
| QTP/UFT                                                                                                                                   | Selenium                                                                                                           | ویژگی                                     |
|------------------------------------------------------------------------------------------------------------------------------------------:|-------------------------------------------------------------------------------------------------------------------:|------------------------------------------:|
| برنامه دسکتاپ                                                                                                                             | مجموعه‌ای از APIها                                                                                                 | نوع نرم‌افزار                             |
| فقط می‌تواند آزمون‌ها را در نسخه‌های خاصی از Google Chrome ، Mozilla Firefox و Internet Explorer انجام دهد.                               | در تمام مرورگرها اجرا می‌شود                                                                                       | مرورگرهای پشتیبانی شده                    |
| دارای license  و تجاری                                                                                                                    | Open-source و رایگان                                                                                               | License (مجوز)                            |
| برای اجرای موازی آزمون‌ها باید با Micro Focus ALM که یک فریم‌ورک غیررایگان است ترکیب شود.                                                 | می‌تواند آزمون‌ها را به‌صورت موازی با ادغام فریم‌ورک‌های آزمون مانند TestNG اجرا کند.                              | اجرای آزمون                               |
| به منابع سیستم بیشتری نیاز دارد. فقط می‌تواند بر روی Windows VM اجرا شود که از منابع بیشتری استفاده می‌کند و نیاز به نگهداری بیشتری دارد. | به منابع سیستم کمتری نیاز دارد و می‌تواند در Windows  یا Linux VM  استفاده شود. Linux VM سبک‌تر از Windows VM است. | مصرف منابع در حین اجرای اسکریپت‌های آزمون |
| فقط از VBScript پشتیبانی می‌کند؛ بنابراین، رویکرد شی گرا برای خودکارسازی با QTP/UFT چالش‌برانگیز می‌شود.                                  | از زبان‌های شیءگرا مانند java  و دیگر زبان‌ها مانندC#, Ruby, Python Perl, PHP ,JavaScript پشتیبانی می‌کند.         | زبان برنامه‌نویسی                         |
| فقط Windows                                                                                                                               | Android, iOS, Windows, Linux, Mac, Solaris.                                                                        | محیط‌های پشتیبانی شده              

### مزایای QTP نسبت به Selenium:
| QTP/UFT                                                                                                                                                   | Selenium                                                                                                           | ویژگی                          |
|----------------------------------------------------------------------------------------------------------------------------------------------------------:|-------------------------------------------------------------------------------------------------------------------:|-------------------------------:|
| Repository  داخلی دارد.                                                                                                                                   | چنین repositoryی ندارد.                                                                                            | Object Repository              |
| از خودکارسازی آزمون برنامه‌های وب و همچنین به‌واسطه راهکار HP که HP Mobile Center نامیده می‌شود، از خودکارسازی آزمون برنامه‌های موبایل پشتیبانی می‌کند.   | خودکارسازی آزمون نرم‌افزارهای موبایلی را پشتیبانی نکرده و فقط می‌تواند اپلیکیشن‌های وب را مورد آزمون قرار دهد.     | پشتیبانی از اپلیکیشن           |
| به دلیل اینکه یک IDE با امکانات کامل است، خودکارسازی را سریع‌تر از Selenium  انجام می‌دهد.                                                                | به دلیل اینکه فاقد یک IDE محلی است و فقط IDE ثالث می‌تواند برای توسعه استفاده شود، سرعت خودکارسازی پایین‌تری دارد. | سرعت خودکارسازی                |
| تقریباً از هر برنامه و محیط نرم‌افزاری از جمله SAP، Oracle، Salesforce، mainframeها ، فریم‌ورک‌های تعبیه شده، مرورگرهای headless   و ... پشتیبانی می‌کند. | در حین خودکارسازی برنامه‌های SAP، Salesforce، Mainframe  با مشکل مواجه می‌شود.                                     | پشتیبانی از تکنولوژی           |
| کنترل‌هایی مانند favourite bar،  دکمه‌های عقب و جلو در مرورگر قابل‌دسترسی هستند.                                                                          | فقط ناحیه قابل‌مشاهده مرورگر که در آن صفحه بارگذاری می‌شود را کنترل می‌کند.                                        | کنترل مرورگر                   |
| ارائه پشتیبانی حرفه‌ای به مشتریان                                                                                                                         | هیچ پشتیبانی رسمی برای کاربران ارائه نشده است.                                                                     | گارانتی                        |
| تولید گزارش آزمون داخلی درون ابزار                                                                                                                        | برای تولید گزارش‌های آزمون وابسته به ابزار خارجی است.                                                              | تولید گزارش آزمون              |
| ارائه ابزارهایی داخلی برای پارامترسازی                                                                                                                    | برای پارامترسازی باید یکی از زبان‌های برنامه‌نویسی پشتیبانی شده را استفاده کنید.                                   | Parameterization (پارامترسازی) |
|                                                                                                                                               

در نهایت، این یک بررسی کلی از تفاوت بین ابزارهای خودکارسازی آزمون Selenium و QTP است. اگرچه QTP دارای قابلیت‌های بیشتری است، اما سلنیوم در هزینه، انعطاف‌پذیری و آزمون موازی غالب بوده و اگر از آن به‌عنوان یک ابزار خودکارسازی آزمون عملکرد استفاده کنیم، می‌توانیم هزینه مورداستفاده برای مجوز QTP را حذف کنیم. 
